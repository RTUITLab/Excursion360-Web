...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const configurationResponse = await fetch(BuildConfiguration.ConfigFilePath || "config.json");
    ...
    try {
        ...
        const excursion = await (await fetch(configuration.sceneUrl + "tour.json")).json() as Excursion;
        ...
        if (excursion.tourProtocolVersion != supportedTourVersion) {
            alert(`That viewer supports only tour ${supportedTourVersion}, please use another viewer or builder (now try ${excursion.tourProtocolVersion})`);
            return;
        }
        console.table({

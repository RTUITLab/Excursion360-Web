...
import type { CroppedImage } from "./Models/ExcursionModels/CroppedImage";
import type { Excursion } from "./Models/ExcursionModels/Excursion";
import type { State } from "./Models/ExcursionModels/State";
import type { FieldItem } from "./Models/FieldItem";
import { FieldItemInfo } from "./Models/FieldItemInfo";
import type { GroupLink } from "./Models/GroupLink";
...
export class Viewer {
	...
	public createScene(minFov?: number, maxFov?: number) {
		...
		if ("xr" in window.navigator) {
			import("./AsyncModules/WebXRLogic").then((module) => {
				module.WebXRLogic.CreateXR(
					scene,
					this.configuration.forceInputProfileWebXr,
					{
						audioButtonPressed: () => {
							this.backgroundAudio.togglePlayPause();
						},
	...
	private async drawImage(
		...
		// biome-ignore lint/suspicious/noAsyncPromiseExecutor: Из-за того что не умеем корректно работать с assetManager вот такое веселье имеет место
